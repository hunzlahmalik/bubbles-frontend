...
export interface PricingCardProps extends CardProps {
  title: string;
...
export const PriceCard: React.FC<PricingCardProps> = ({ title, amount, coinUrl, usd, ...props }) => {
  return (
    <>
      <StyledPriceCard {...props}>
        ...
        <PriceCardIcon {...props}>
          {coinUrl ? (
            <img style={{ height: '24px', width: '24px', marginTop: '10px' }} alt="nothing" src={coinUrl} />
          ) : (
            ''
          )}
